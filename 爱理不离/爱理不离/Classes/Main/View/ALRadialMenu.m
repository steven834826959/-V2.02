...
- (void)itemsWillAppearFromButton:(UIButton *) button withFrame:(CGRect)frame inView:(UIView *)view {
	...
	while (currentItem <= itemCount) {
		...
		int extraX = round (centerX + (radius) * cos(radians));
		int extraY = round (centerY + (radius) * sin(radians));
	...
	float spinDuration = ([self.items count]+.5f) * flingInterval;
...
- (void)itemsWillDisapearIntoButton:(UIButton *) button {
    ...
    //隐藏外环View
    [[NSNotificationCenter defaultCenter]postNotificationName:@"circleViewHidden" object:nil];
