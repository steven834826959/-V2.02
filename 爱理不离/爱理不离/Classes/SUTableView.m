...
- (void)resetContentOffsetIfNeeded {
    ...
    if (contentOffset.y < 0.0) {
        contentOffset.y = self.contentSize.height / 1000000.0f;
    }
    //scroll over bottom
    else if (contentOffset.y >= (self.contentSize.height - self.bounds.size.height)) {
        contentOffset.y = self.contentSize.height / 1000000.0f - self.bounds.size.height;
    }
...
- (NSInteger)tableView:(UITableView*)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.actualRows * 1000000;
}
